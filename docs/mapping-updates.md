...
Run **Update reward mappings** from the repository's Actions tab. **Use
workflow from** selects the exporter branch whose updater is run and the target
branch for the resulting pull request. The `source_ref` input separately
selects an NTE_Assets branch, tag, or commit and defaults to `main`.
